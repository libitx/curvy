defmodule Curvy.Key do
  ...
  """
  @spec from_privkey(binary) :: t
  def from_privkey(<<privkey::binary>>) do
  ...
  def from_pubkey(<<_::integer, x::big-size(256), y::big-size(256)>>),
    do: %__MODULE__{point: %Point{x: x, y: y}}
  ...
  * `:compressed` - Return a 32 byte compressed public key. Default is `true`.
  """
  def to_pubkey(%__MODULE__{point: %Point{x: x, y: y}}, opts \\ []) do
    case Keyword.get(opts, :compressed, true) do
      true ->
